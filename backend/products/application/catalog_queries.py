...
def apply_catalog_filters(queryset: QuerySet[Product], params) -> QuerySet[Product]:
    ...
    if category:
        ...
        gender_tag = GENDER_CATEGORY_TAGS.get(category_key)
        if gender_tag:
            category_filter = _tag_filter(gender_tag)
            if params.get('include_unisex', 'true') in {'true', '1'}:
                category_filter |= _tag_filter('unisex')
        else:
