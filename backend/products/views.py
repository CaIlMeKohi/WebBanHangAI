...
class AuthLoginAPIView(APIView):
    def post(self, request):
        ...
        if not user or not user.is_active:
            _log_login_attempt(request, user, username, False, 'inactive_or_missing')
            return Response({'detail': 'Tai khoan khong hop le hoac da bi khoa'}, status=status.HTTP_400_BAD_REQUEST)
        if not check_password(password, user.password_hash):
            _log_login_attempt(request, user, username, False, 'bad_password')
            return Response({'detail': 'Invalid credentials'}, status=status.HTTP_400_BAD_REQUEST)

        _log_login_attempt(request, user, username, True, '')
...
def _log_login_attempt(request, user, identifier, success, reason):
    try:
